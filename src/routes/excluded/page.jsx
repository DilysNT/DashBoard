...
const ExcludedServicesPage = () => {
  ...
  const API_BASE_URL = "http://localhost:5000/api/excluded-services";

  // Fetch excluded services from API
  const fetchServices = async () => {
    try {
      ...
      console.error("Error fetching excluded services:", error);
      setErrorMessage("Không thể tải danh sách dịch vụ loại trừ. Vui lòng thử lại!");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createService = async (serviceData) => {
    try {
      ...
      console.error("Error creating excluded service:", error);
      setErrorMessage(error.message || "Không thể tạo dịch vụ loại trừ mới. Vui lòng thử lại!");
      return false;
    }
  };

  // Update existing excluded service
  const updateService = async (id, serviceData) => {
    try {
      ...
      console.error("Error updating excluded service:", error);
      setErrorMessage(error.message || "Không thể cập nhật dịch vụ loại trừ. Vui lòng thử lại!");
      return false;
    }
  };

  // Delete excluded service
  const deleteService = async (id) => {
    try {
      ...
      console.error("Error deleting excluded service:", error);
      setErrorMessage(error.message || "Không thể xóa dịch vụ loại trừ. Vui lòng thử lại!");
      return false;
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa dịch vụ loại trừ này?")) {
    ...
    return (
      <div className="p-6 flex justify-center items-center min-h-screen">
        <div className="flex items-center gap-2">
          ...
          <span>Đang tải danh sách dịch vụ loại trừ...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Quản lý Dịch Vụ Loại Trừ</h1>
          <p className="text-slate-600 mt-1">Quản lý các dịch vụ không bao gồm trong tour</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl transform transition-all duration-300 ease-in-out">
            <h2 className="text-2xl font-semibold text-gray-900 mb-6 text-center">
              {editingService ? "Sửa Dịch Vụ Loại Trừ" : "Thêm Dịch Vụ Loại Trừ Mới"}
            </h2>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ví dụ: Không bao gồm bảo hiểm du lịch"
                />
              </div>
            ...
            {filteredServices.length === 0 ? (
              <tr>
                <td colSpan={3} className="p-6 text-center text-sm text-slate-500">
                  {searchTerm ? "Không tìm thấy dịch vụ loại trừ phù hợp." : "Chưa có dịch vụ loại trừ nào."}
                </td>
              </tr>
            ) : (
                ...
                <tr key={service.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 text-sm text-slate-700">
                    <div className="font-mono text-xs bg-slate-100 px-2 py-1 rounded">
                      {typeof service.id === 'string' ? service.id.substring(0, 8) + '...' : service.id}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm font-medium text-slate-900 max-w-md">
                    <div className="truncate" title={service.service_name}>
...
export default ExcludedServicesPage;