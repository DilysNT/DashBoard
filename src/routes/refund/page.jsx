...
const REFUND_API_URL = "http://localhost:5000/api/payments/refund"; // API cho admin
...
const RefundAdminPage = () => {
    ...
    // Filter theo lý do hoàn tiền
    let filteredRefunds = refunds;
    if (filterSource !== "all") {
        filteredRefunds = filteredRefunds.filter(r => r.source === filterSource);
    }
    if (filterReason !== "all") {
        filteredRefunds = filteredRefunds.filter(r => r.reason && r.reason.toLowerCase().includes(filterReason));
    }
    ...
    return (
        <div className="p-6 flex flex-col gap-6">
            <h1 className="text-3xl font-bold text-slate-900 mb-2">Quản lý hoàn tiền (Admin)</h1>
           ...
           {/* Filter UI */}
            <div className="flex gap-4 items-center mb-2">
                ...
                <label className="font-medium ml-4">Lý do hoàn tiền:</label>
                <select value={filterReason} onChange={e => { setFilterReason(e.target.value); setCurrentPage(1); }} className="border rounded px-2 py-1">
                    <option value="all">Tất cả</option>
                    <option value="user">User yêu cầu</option>
                    <option value="agency">Agency yêu cầu</option>
                </select>
            </div>
            {error && (
